---
title: Appendix
numbering:
  title:
    enabled: false

---

```{card}
The physics content is covered in the previous chapters. To let you study (more) independently, we present here the mathematics involved. We do not describe in detail the required mathematical concepts, we merely provide a summary of the main concepts, together with some code snippets to illustrate how to implement them in Python. We align with the approach in the TU Delft books: 
* [Linear Algebra](https://interactivetextbooks.tudelft.nl/linear-algebra/)
* [Calculus](https://tudelft-prime-books.github.io/Calculus/)

However, here we use the notations and quantities used in this book.

We also provide an example exam and additional grasple exercises.
```